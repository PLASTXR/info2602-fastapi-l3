...
def initialize():
    with get_session() as db: # Get a connection to the database
        ...
        new_todo = Todo(text='Wash dishes', user_id = bob.id or 0) # Create a new todo (in memory)
...
def add_task(username:str, task:str):
    with get_session() as db:
        ...
        user.todos.append(Todo(text=task, user_id = user.id or 0))
        db.add(user)
        db.commit()
        print("Task added for user")
        
@cli.command()
def check_user_todos(username:str):
    with get_session() as db:
        user = db.exec(select(User).where(User.username == username)).one_or_none()
        if not user:
            print("User doesn't exist")
        ...
        for record in todo:
            print(f"Todo id: {record.id}, text: {record.text}, done: {record.done}")
...
def create_category(username:str, cat_text:str):
    with get_session() as db: # Get a connection to the database
        ...
        category = Category(text=cat_text, user_id=user.id or 0)
        db.add(category)
        db.commit()
